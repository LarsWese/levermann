...
def scrape_share_data(share: Share):
    ...
    :return:
    """
    logger.info(f'get data for {share.isin}')
    ...
    if len(share_raw_data) <= 0:
        return
    __map_share_data(share, share_raw_data)

    logger.info(f"Get metrics for {share.isin}")
...
def __map_share_data(share: Share, share_meta_datas: [RawData]):
    """
    :param today:
